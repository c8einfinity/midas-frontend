...
define([
    ...
    return Component.extend({
        ...
        initObservable: function () {
            this._super().observe(
                'actions opened attributes dropdowns productMatrix value usedAttributes attributesData attributeCodes'
            );
         ...
         * @param {*} dropdowns
         */
        render: function (variations, dropdowns) {
            this.changeButtonWizard();
            this.populateVariationMatrix(variations);
            this.attributes(dropdowns);
            this.dropdowns(dropdowns);
            this.disableConfigurableAttributes(dropdowns);
        ...
        handleValue: function (variations) {
            ...
            _.each(variations, function (variation) {
                var attributes = _.reduce(variation.options, function (memo, option) {
                        ...
                        attribute[option['dropdown_label']] = option.dropdown_id;
        ...
        handleDropdowns: function () {
            ...
            _.each(this.dropdowns(), function (dropdown) {
                tmpArray.push(dropdown.id);
                codesArray.push(dropdown.code);
                values = {};

                _.each(dropdown.chosen, function (row) {
                    values[row.id] = {
                        'include': '1',
                        'value_index': row.id
                    };
         ...
         *   - finder_product_ids_serialized.
         */
        serializeData: function () {
            this.source.data['finder-matrix-serialized'] =
                JSON.stringify(this.source.data['finder-matrix']);

            delete this.source.data['finder-matrix'];

            this.source.data['finder_product_ids_serialized'] =
                JSON.stringify(this.source.data['finder_product_ids']);

            delete this.source.data['finder_product_ids'];
        },
