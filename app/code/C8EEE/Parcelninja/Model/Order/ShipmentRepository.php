...
namespace C8EEE\Parcelninja\Model\Order;
...
class ShipmentRepository implements \Magento\Sales\Api\ShipmentRepositoryInterface
{
    ...
    {
        //Search by the tracking number first then fall back to order id
        ...
        if (isset($_REQUEST["searchCriteria"]["filterGroups"])) {
            $orderIdOrTracknumber = $_REQUEST["searchCriteria"]["filterGroups"][0]["filters"][0]["value"];
            $searchResult->getSelect()
                ->join(array('sales_shipment_track' => 'sales_shipment_track'), 'main_table.order_id = sales_shipment_track.order_id', 'sales_shipment_track.track_number')
                ->where("(sales_shipment_track.track_number = '$orderIdOrTracknumber')");
        }
         else {
